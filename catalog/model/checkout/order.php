...
class ModelCheckoutOrder extends Model
{
	...
	public function getOrdersToExpire()
	{
		...
		$sql = "SELECT
					*
					...
					oc_order AS o
					WHERE
					TIMESTAMPDIFF(HOUR,o.date_added,now()) >= $max_hours AND 
					o.order_status_id = 1";
	...
	public function expireOrder($order)
	{
		$expire_status = 14;
		///UPDATE STATUS
		$this->db->query("UPDATE " . DB_PREFIX . "order SET order_status_id = '" . (int) $expire_status . "' WHERE order_id = '" . (int) $order["order_id"] . "'");
		///HISTORY
		$this->db->query("INSERT INTO " . DB_PREFIX . "order_history SET order_id = '" . $order["order_id"] . "',
			 order_status_id = '" . $expire_status . "',
			 notify = 1,
			 comment = 'ORDEN EXPIRADA POR FALTA DE PAGO',
			 date_added = NOW(); ");
	}
}
